class Garden {
  ...
  ripenPlant(plantName, quantity) {
    let currentPlant = this.plants.findIndex((x) => x.plantName == plantName);
    if (currentPlant == -1) {
      throw new Error(`There is no ${plantName} in the garden.`);
    }
    if (this.plants[currentPlant].ripe) {
      throw new Error(`The ${plantName} is already ripe.`);
    }
    if (quantity <= 0) {
      throw new Error("The quantity cannot be zero or negative.");
    }
    this.plants[currentPlant].ripe = true;
    this.plants[currentPlant].quantity += quantity;
    if (quantity == 1) {
  ...
  harvestPlant(plantName) {
    ...
    let harvestedPlant = {
      plantName,
      quantity: this.plants[currentPlant].quantity,
    };
    this.spaceAvailable += this.plants[currentPlant].spaceRequired;
    this.plants.splice(currentPlant, 1);
