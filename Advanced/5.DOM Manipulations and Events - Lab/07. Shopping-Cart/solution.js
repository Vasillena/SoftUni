function solve() {
  ...
  function add(event) {
    ...
    textOutput = `Added ${current} for ${products[current].toFixed(
      2
    )} to the cart.\n`;
    totalPrice += products[current];
    textArea.value += textOutput;
    if (!listOfProducts.includes(current)) {
      listOfProducts.push(current);
    }
  }

  function checkout() {
    textOutput = `You bought ${listOfProducts.join(
      ", "
