class Company {
  ...
  bestDepartment() {
    ...
    let sorted = Object.entries(this.departments[theBestDepart]).sort(
      (a, b) => b[1].salary - a[1].salary || a[0].localeCompare(b[0])
    );
    sorted.forEach((el) =>
      theBestWithData.push(`${el[0]} ${el[1].salary} ${el[1].position}`)
    );
