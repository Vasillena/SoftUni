...
const newStudentsTemplate = (data) => html` ${data.map(
  (student) => html`
    <tr id=${student._id}>
...
function onClick() {
  ...
  if (textInput !== "") {
    ...
    for (const student of dataFromServer) {
      ...
      if (
        email.includes(textToLow) ||
        firstName.includes(textToLow) ||
        lastName.includes(textToLow) ||
        course.includes(textToLow)
      ) {
        document
          .getElementById(`${student._id}`)
